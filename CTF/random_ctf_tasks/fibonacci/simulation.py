...
def hook_code(mu, address, size, user_data):  
    ...
    if address in instructions_skip_list:
        ...
        if (arg0,arg1) in d:                        # Check whether return values for this function are already saved.
            ...
            # more 'elegant' way below
            r_rsp = mu.reg_read(UC_X86_REG_RSP)     # Read RSP
            ret_addr = u64(mu.mem_read(r_rsp, 8))   # Read return address from stack
            mu.reg_write(UC_X86_REG_RIP, ret_addr)  # Set RIP to return address 
            mu.reg_write(UC_X86_REG_RSP, r_rsp + 8) # Pop return address from stack
